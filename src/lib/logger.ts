...
function emitLog(level: LogLevel, payload: LogPayload) {
  ...
  if (payload.error) {
    logObject.error = payload.error instanceof Error ? payload.error.message : String(payload.error);
    logObject.stack = payload.error instanceof Error ? payload.error.stack : undefined;
  }
