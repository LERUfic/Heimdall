    ...
    if (cloneData) {
      try {
        ...
        if (r.headers) {
          ...
          if (hArr.length > 0) {
            setHeadersArr([...hArr, { key: '', value: '' }])
          }
        }
      } catch(e) {}
      sessionStorage.removeItem('clone_request')
    }
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validParams.length > 0) {
      try {
        ...
        // If URL parsing fails (e.g. invalid URL), we fallback to manual append
    ...
    <div className="min-h-screen bg-[#1c1c1c] p-6 text-zinc-300 font-sans">
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <span className="text-zinc-300 text-sm font-medium">New Request</span>
        </div>
